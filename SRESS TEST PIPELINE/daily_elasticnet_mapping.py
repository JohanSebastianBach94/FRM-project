#!/usr/bin/env python3
"""Daily ElasticNet driver mapping for the numbered Step 6 pipeline (6.2)."""
...
def _load_country_blocks() -> List[Dict]:
    ...
    try:
        with BLOCK_CONFIG.open("r", encoding="utf-8") as fp:
            return (yaml.safe_load(fp) or {}).get("country_blocks", [])
    except FileNotFoundError:
        return []


COUNTRY_BLOCKS = _load_country_blocks()
...
def run_lasso_for_iso(
    ...
    *,
    ...
    feature_names = [c for c in panel.columns if c != y_series.name]
    ...
    pd.DataFrame({"alphas": enet_cv.alphas_, "mse_path_mean": enet_cv.mse_path_.mean(axis=1)}).to_csv(cv_path, index=False)
    ...
    coefs = pd.Series(enet_cv.coef_, index=feature_names)
