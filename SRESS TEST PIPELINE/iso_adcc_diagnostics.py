"""Step 6.5 ISO-level GARCH + ADCC diagnostics, now living inside SRESS TEST PIPELINE.

It follows the implementation plan: it loads the standardized factor shortlists, fits
...
def nearest_spd_correlation(R: np.ndarray, eps: float = 1e-10) -> np.ndarray:
...
def ensure_valid_correlation(R: np.ndarray, eps: float = 1e-10) -> tuple[np.ndarray, bool]:
    R = np.asarray(R, dtype=float)
    R = (R + R.T) / 2
    ...
    try:
        ...
        return nearest_spd_correlation(R, eps=eps), True
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Recompute even if ISO ADCC artifacts already exist on disk.",
    )
    parser.add_argument(
        "--model",
        choices=["adcc", "dcc"],
    ...
    parser.add_argument(
        ...
        choices=["opt", "grid"],
        default="opt",
        help="Fit ADCC/DCC parameters via constrained optimization (opt) or coarse grid search (grid)",
    )
    parser.add_argument(
        "--opt-maxiter",
        type=int,
        default=250,
        help="Maximum iterations for constrained optimizer when --fit-method=opt",
    )
    parser.add_argument(
        "--max-a",
        type=float,
        default=None,
        help="Optional upper bound for DCC/ADCC 'a' (e.g., 0.2).",
    )
    parser.add_argument(
        "--large-a-penalty-lambda",
        type=float,
        default=0.0,
        help="Optional quadratic penalty strength for large 'a' (distinct from a+b stationarity).",
    )
    parser.add_argument(
        "--large-a-threshold",
        type=float,
        default=0.2,
        help="Threshold above which large-a penalty activates (default: 0.2).",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        "--min-garch-convergence-rate",
        type=float,
        default=0.5,
        help=(
            "Quality gate: if fraction of GARCH fits converged is below this threshold, "
            "skip ADCC/DCC fitting and force shrinkage correlation instead. "
            "Set to 0 to disable."
        ),
    )
    parser.add_argument(
...
def drop_low_coverage_series(data: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    dropped = sorted(low_coverage.index)
    if dropped:
        print(
            f"  Dropping {len(dropped)} low-coverage factors (<{threshold:.2f}): "
            f"{', '.join(dropped)}"
        )
    kept = [col for col in coverage.index if coverage[col] >= threshold]
    for series in sorted(REQUIRED_SERIES):
        if series in coverage:
            print(f"  coverage for {series}: {coverage[series]:.4%}")
    if len(kept) < 2:
...
def evaluate_low_autocorr_subset(
    ...
    params, loglik, _fit_info = fit_adcc_params(
        subset,
    ...
    for column in data.columns:
        ...
        if col_path.exists() and not overwrite:
            try:
                ...
                converged_flag = bool(model.get("diagnostic", {}).get("converged", True))
                diagnostics.append(
                    dict(
                        iso=iso,
                        series=column,
    ...
    for t in range(1, T):
        ...
        try:
            ...
            if sign <= 0 or not np.isfinite(logdet):
                R_t = nearest_spd_correlation(R_t)
                sign, logdet = np.linalg.slogdet(R_t)
                if sign <= 0 or not np.isfinite(logdet):
                    return -np.inf
            quad = float(u_t.T @ np.linalg.solve(R_t, u_t))
            loglik += -0.5 * (float(logdet) + quad)
        except np.linalg.LinAlgError:
            return -np.inf
    return loglik
...
def grid_search_adcc(
    ...
    if max_a is not None:
        a_values = [a for a in a_values if a <= float(max_a)]
...
def _stationarity_margin(params: Sequence[float], model_type: str) -> float:
    a, b = float(params[0]), float(params[1])
    g = float(params[2]) if model_type == "adcc" else 0.0
    return 1.0 - a - b - 0.5 * g
...
def fit_adcc_params(
    ...
    opt_maxiter: int = 250,
    max_a: Optional[float] = None,
    large_a_penalty_lambda: float = 0.0,
    large_a_threshold: float = 0.2,
) -> Tuple[Tuple[float, float, float], float, Dict[str, Any]]:
    """Fit (a,b,g) for DCC/ADCC.

    - `fit_method="opt"` uses constrained optimization enforcing:
        a>=0, b>=0, g>=0 and a+b+0.5g < 1.
    - Falls back to coarse grid search when optimization fails.
    """
    ...
    if fit_method == "grid":
        params, loglik = grid_search_adcc(residuals, model_type, grid_a, grid_b, grid_g, max_a=max_a)
        fit_info["grid"] = {
            "used": True,
            "grid_a": list(grid_a) if grid_a else None,
            "grid_b": list(grid_b) if grid_b else None,
            "grid_g": list(grid_g) if grid_g else None,
            "max_a": max_a,
            "large_a_penalty_lambda": float(large_a_penalty_lambda),
            "large_a_threshold": float(large_a_threshold),
        }
        return params, loglik, fit_info

    R_bar = residuals.corr()
    eps = 1e-6
    a_upper = float(max_a) if max_a is not None else 0.999
    a_upper = float(np.clip(a_upper, 1e-8, 0.999))
    bounds = [
        (0.0, a_upper),
        (0.0, 0.999),
        (0.0, 0.999 if model_type == "adcc" else 0.0),
    ]

    x0 = np.array([0.05, 0.90, 0.05 if model_type == "adcc" else 0.0], dtype=float)
    if _stationarity_margin(x0, model_type) <= 10 * eps:
        # Ensure feasible initialization (simple repair).
        x0[1] = max(0.0, min(x0[1], 0.95 - x0[0] - 0.5 * x0[2]))
        if model_type != "adcc":
            x0[2] = 0.0

    def objective(x: np.ndarray) -> float:
        ll = dcc_loglikelihood((x[0], x[1], x[2]), residuals, R_bar, model_type)
        if not np.isfinite(ll):
            return 1e12
        a_val = float(x[0])
        penalty = (
            float(large_a_penalty_lambda) * max(0.0, a_val - float(large_a_threshold)) ** 2
            if float(large_a_penalty_lambda) > 0
            else 0.0
        )
        return -float(ll) + float(penalty)

    constraints = [
        {
            "type": "ineq",
            "fun": lambda x: _stationarity_margin(x, model_type) - eps,
        }
    ]
    ...
    try:
        result = minimize(
            ...
            options={"maxiter": int(opt_maxiter), "ftol": 1e-9},
        )
        fit_info["opt"] = {
        ...
        if bool(getattr(result, "success", False)) and getattr(result, "x", None) is not None:
            ...
            loglik = dcc_loglikelihood(params, residuals, R_bar, model_type)
            if np.isfinite(loglik):
                return params, float(loglik), fit_info
    except Exception as exc:
        fit_info["opt"] = {"success": False, "message": f"exception: {exc}"}

    params, loglik = grid_search_adcc(residuals, model_type, grid_a, grid_b, grid_g, max_a=max_a)
    fit_info["fallback"] = "grid"
    return params, loglik, fit_info
...
def compute_dynamic_correlations(
    ...
    for t in range(1, T):
        ...
        R_t, projected = ensure_valid_correlation(R_t, eps=1e-10)
        if projected:
    ...
    return corr_df, persistence, {"spd_projection_dates": int(spd_projection_dates)}
...
def main() -> None:
    ...
    for iso in isos:
        expected_artifacts = [
            DIAG_DIR / f"garch_diagnostics_{iso}.csv",
            DIAG_DIR / f"garch_standardized_residuals_{iso}.csv",
            DIAG_DIR / f"garch_conditional_vols_{iso}.csv",
            DIAG_CORR_DIR / f"{iso}_Rt.csv",
            DIAG_CORR_DIR / f"{iso}_Sigma_pairs.csv",
            DIAG_CORR_DIR / f"{iso}_Sigma_eigenvalues.csv",
            ADCC_MODELS_DIR / f"{iso}_adcc.json",
        ]
        if not args.force and all(path.exists() for path in expected_artifacts):
            print(f"\n=== Processing {iso} ===")
            print(f"[SKIP] ISO ADCC artifacts already exist for {iso} (use --force to recompute).")
            continue
        ...
        # GARCH quality gate: if too many series fail to converge, do not attempt
        # ADCC/DCC on (potentially garbage) standardized residuals.
        min_conv = float(args.min_garch_convergence_rate)
        if min_conv > 0:
            if "converged" not in diag_df.columns:
                diag_df["converged"] = False
            conv_rate = float(np.mean(diag_df["converged"].fillna(False).astype(bool))) if not diag_df.empty else 0.0
        else:
            conv_rate = float("nan")
        ...
        gate_tripped = bool(min_conv > 0 and np.isfinite(conv_rate) and conv_rate < min_conv)
        fit_info: Optional[Dict[str, Any]] = None

        if gate_tripped:
            print(
                f"  GARCH quality gate: convergence {conv_rate:.2%} < {min_conv:.2%}; "
                "forcing shrinkage correlation (skip ADCC/DCC)"
            )
            ...
            fit_info = {
                "fallback": {
                    "reason": "low_garch_convergence",
                    "garch_convergence_rate": conv_rate,
                    "min_garch_convergence_rate": min_conv,
            ...
            fit_info = {
                "fallback": {
                    "reason": "insufficient_residual_rows",
                    "residual_rows": int(len(cleaned)),
                    "min_residual_rows": int(args.min_residual_rows),
                    "shrinkage": float(args.shrinkage),
                    "garch_convergence_rate": conv_rate,
                    "min_garch_convergence_rate": min_conv,
                }
            ...
            params, loglik, fit_info = fit_adcc_params(
                ...
                max_a=args.max_a,
                large_a_penalty_lambda=args.large_a_penalty_lambda,
                large_a_threshold=args.large_a_threshold,
            )
            corr_df, persistence, corr_info = compute_dynamic_correlations(cleaned, args.model, params)
            model_label = args.model
            fit_info["corr_recursion"] = corr_info
            fit_info.setdefault("garch", {})
            fit_info["garch"].update(
                {
                    "garch_convergence_rate": conv_rate,
                    "min_garch_convergence_rate": min_conv,
                }
