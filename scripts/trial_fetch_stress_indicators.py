...
try:
    ...
    try:
        ...
        print(f"⚠️  FRED API key issue: {e}")
    ...
    if fred:
        ...
        for i, code in enumerate(series_codes, 1):
            try:
                ...
                print(f"\n   ⚠️  Failed to fetch {code}: {str(e)[:50]}")
                continue
...
# ==============================================
# 🛢️ STEP 2: FETCH YAHOO FINANCE COMMODITIES
# ==============================================
...
try:
    ...
    for i, (ticker, meta) in enumerate(ALL_YAHOO_SERIES.items(), 1):
        try:
            ...
            print(f"\n   ⚠️  Failed to fetch {ticker}: {str(e)[:50]}")
            continue
...
if not fred_df.empty:
    ...
    for spread_name, spread_info in COMPUTED_SPREADS.items():
        try:
            ...
            if all(comp in fred_df.columns for comp in components):
                ...
                print(f"   ⚠️  {spread_name}: Missing components {missing}")
    ...
    print("⚠️  No FRED data available for computing spreads")
